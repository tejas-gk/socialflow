...
export default function PinterestCallbackInner() {
        ...
        try {
            ...
            if (data.access_token) {
                ...
                if (data.refresh_token) {
                    localStorage.setItem("pinterest_refresh_token", data.refresh_token)
                }
                    ...
                    if (window.opener) {
                        try {
                            window.opener.postMessage(
                                { type: "pinterest_oauth_success", token: data.access_token },
                ...
                if (window.opener) {
                    try {
                        window.opener.postMessage(
                            { type: "pinterest_oauth_error", error: "Failed to get access token" },
                            window.location.origin
                        )
                    } catch (e) {
                        console.error("Failed to send postMessage:", e)
            ...
            if (window.opener) {
                try {
                    window.opener.postMessage(
                        { type: "pinterest_oauth_error", error: error instanceof Error ? error.message : "Authentication failed" },
                        window.location.origin
                    )
                } catch (e) {
                    console.error("Failed to send postMessage:", e)
